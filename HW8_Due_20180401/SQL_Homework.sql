## Homework Assignment
/*
SQL Homework, 2018 March, Tony Li
*/
USE sakila;
...
-- This is the join method, was faster than the subquery method (0.000 sec)
SELECT c.email
FROM customer c
...
/* This is the subquery method, took a little longer (0.016 sec)
SELECT email
FROM customer
WHERE address_id IN
(
...
SELECT s.store_id, a.address, ci.city, cn.country
FROM store s
INNER JOIN address a
...
CREATE VIEW top_five_genres_gr AS
SELECT c.name, SUM(p.amount)
FROM category c
INNER JOIN film_category fc
ON c.category_id = fc.category_id
...
SELECT * FROM top_five_genres_gr;
-- * 8c. You find that you no longer need the view `top_five_genres`. Write a query to delete it.
DROP VIEW top_five_genres_gr;